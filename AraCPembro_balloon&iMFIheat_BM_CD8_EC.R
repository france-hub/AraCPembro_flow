...
load("workspace_CD8_APembro.rds")
...
freq_table <- freq_table[c(5,1,2,6,4,7,3),]
...
matrix_r <- matrix_r[-c(1,9:10), ]
colnames(matrix_r)
col_later <- gsub(".fcs", "", matrix_r$X)

matrix_r <- matrix_r[, c(2:17)]
colnames(matrix_r)
...
iMFI <- freqs*MFI
colnames(iMFI) <- c("CCR7", "CD27", "CD28", "CD45RA", "CD57", "DNAM1", "KRLG1", "PD1")
scaled_iMFI <- iMFI
...
heatmap.2(as.matrix(t(scale.data)),
          ...
          xlab="Markers", ylab= "CLUSTERS", cexRow = 0.8)
