...
#load("AraCPembro_step1_CD4_PB_EC.rds")
...
plotExprHeatmap(sce, features = "type",
                by = "cluster_id", k = "meta13", 
                bars = TRUE, perc = TRUE, col_anno = TRUE, scale = "last", bin_anno = TRUE)

#Filter cluster < 1%
sce <- filterSCE(sce, cluster_id %in% c(1,3,4,5,7,8,9,10,13), k = "meta13")
...
annotation_table <- readxl::read_excel("Annotation_CD4_PB_EC.xlsx")
annotation_table
...
sce <- mergeClusters(sce, k = "meta13", 
                     table = annotation_table, id = "cluster_annotation", overwrite = TRUE)

# Plot heatmap with annotations
plotExprHeatmap(sce, features = "type", 
                by = "cluster_id", k = "meta13", m = "cluster_annotation", scale = "last", bin_anno = TRUE)
...
p.adj.signif <- c(rep("ns", 6), "1.0e-02", rep("ns", 2))
y.position <- c(73, 90, 73, 70, 73, 73, 70, 60, 60)
group1 <- (rep("DG_CR",9))
group2 <- (rep("DG_NR", 9))
...
p.adj.signif <- c(rep("ns", 6), "2.2e-03", rep("ns", 2))
y.position <- c(73, 90, 73, 58, 73, 73, 60, 60,60)
group1 <- (rep("post_CR",9))
group2 <- (rep("post_NR", 9))
...
# save workspace 
save(list = ls(), file = "AraCPembro_step2_CD4_PB_EC.rds")
