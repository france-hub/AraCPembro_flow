...
pbMDS(sce, by = "both", k = "meta11", shape_by = "condition", size_by = TRUE)
...
#(not sig)
contrast <- createContrast(c(0, 1, rep(0, 17)))
...
p.adj.signif <- c("1.4e-02", rep("ns", 6))
y.position <- c(58, 90, 58, 58, 58, 58, 58)
group1 <- (rep("yes_onc_sign",7))
group2 <- (rep("no_onc_sign", 7))
stat.test <- cbind(stat.test, group1, group2, p.adj.signif, y.position)
bxp <- plotAbundances(sce, k = "cluster_annotation", by = "cluster_id", group_by = "onc_sign", shape_by = "patient_id")
