...
#load("AraCPembro_step1_CD8_BM_IC.rds")
...
antigen <- fcs_colname
marker_class <- c(rep("type", 4), rep("state",3), rep("type",6), rep("state",2), "type") 
...
fs <- warpSet(fs, stains = antigen[c(4)])
...
#Filter cluster 6 0.51%-aggregate
sce <- filterSCE(sce, cluster_id %in% c(1:5,7:11), k = "meta11")
...
annotation_table <- readxl::read_excel("annotation_notnorm_CD8_IC.xlsx")
annotation_table

# convert to factor with merged clusters in desired order
annotation_table$new_cluster <-factor(annotation_table$new_cluster, 
                                      levels = c("TEMRA CD57+", "T senescent", "T dys effector-like", "T early dys", "T naive", "T dys-1",
                                                 "T naive-like"))
...
##Only one contrast significant
#NR_DG vs NR_post
contrast <- createContrast(c(0, -1, 0, 1, rep(0, 17)))
out_DA <- diffcyt(sce, design = design, contrast = contrast, method_DA = "diffcyt-DA-edgeR",
                  clustering_to_use = "cluster_annotation", verbose = TRUE, subsampling = TRUE, transform = FALSE, min_samples = 17)
topTable(out_DA, format_vals = TRUE)
da <- rowData(out_DA$res)
sub <- filterSCE(sce, condition == c("DG_NR", "post_NR"))
DG_ind <- grep("DG", levels(sub$sample_id))
post_ind <- grep("post", levels(sub$sample_id))
levels(sub$sample_id) <- levels(sub$sample_id)[c(DG_ind, post_ind)]
plotDiffHeatmap(sub, da, all = TRUE, fdr = 0.05, sort_by = "padj", fun = "mean")


#Add sig to boxplots
stat.test <- as_tibble(da)
p.adj.signif <- c("3.7e-02", "5.0e-03", "4.6e-09", "2.9e-02", "ns", "3.0e-02", "ns", "3.8e-02",
                  "2.0e-03", "ns")
y.position <- c(42, 49, 59, 15, 55, 67, 10, 40, 25,60)
group1 <- (rep("DG_NR",10))
